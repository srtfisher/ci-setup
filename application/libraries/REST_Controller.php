...
class REST_Controller extends CI_Controller {
	...
	public function __construct()
	{
		...
		// Set the current user, don't log them in with cookies
          $this->api_user_row = $this->get_api_user();
          
          // Set the user internally
          Auth::set_current_user($this->api_user_row->row()->user_id);
          
          // We don't have to set the permissions because that is detected automatically
          // (it get's it from the Auth library)
          
          // Parent's aren't allowed in the API
          if (Perm::is_parent())
               die('Parents are not permitted to use the API.');
	}
	...
	/**
	 * Enabling people to POST or GET
	 *
	 * We give priority to the GET variables
	 * Essentially, you can put both POST and GET at the same time if you wanted.
	 *
	 * @access    public
	 * @return    string|bool
     **/
     public function param($key = NULL, $xss_clean = TRUE)
     {
	...
	/**
	 * Get the user row for the current API Key
	 * We return the object, not just the row
	 *
	 * @return    object
	 * @access    public
	 * @param     void
     **/
     public function get_api_user()
     {
          ...
          ->from('api_keys')
          ->where('key', $key)
          ->join('users', 'users.user_id = api_keys.user_id AND api_keys.user_id > 0')
          ->get();
     }
}